###############################################################################
## $Id: copilot-dual-control.nas,v 1.5 2010/01/02 09:49:31 vmmeazza Exp $
##
...
#       pilot_type and engineer_type to the name (with full path) of
#       main 3d model XML for the pilot and crew aircrafts.
...
var connect = func (pilot) {
	print("Trying connect pilot to engineer (engineer-dual-control.nas)");
	...
	foreach (var vn; keys(views)) {
		...
		setprop(view_cfg ~ "/target-lat-deg-path", pilot.getNode(DCT.lat_mpp).getPath());
		setprop(view_cfg ~ "/target-lon-deg-path", pilot.getNode(DCT.lon_mpp).getPath());
		setprop(view_cfg ~ "/target-alt-ft-path", pilot.getNode(DCT.alt_mpp).getPath());
		setprop(view_cfg ~ "/target-heading-deg-path", pilot.getNode(DCT.heading_mpp).getPath());
		setprop(view_cfg ~ "/target-pitch-deg-path", pilot.getNode(DCT.pitch_mpp).getPath());
		setprop(view_cfg ~ "/target-roll-deg-path", pilot.getNode(DCT.roll_mpp).getPath());
	}
	...
	process_data = [
		# Map /postition/*
      		DCT.Translator.new(
			pilot.getNode(DCT.lat_mpp),
	 		props.globals.getNode(l_lat)
		),

		DCT.Translator.new(
			pilot.getNode(DCT.lon_mpp),
			props.globals.getNode(l_lon)
		),
    	...
    	] ~ ADC.engineer_connect_pilot(pilot);

	print("Dual control ... connected to pilot.");
	setprop("/sim/messages/engineer", "Welcome aboard. I'm your pilot !");
}
...
var disconnect = func {
	...
	foreach (var vn; keys(views)) {
		...
		if (views[vn] > 0) {
			setprop(view_cfg ~ "/eye-lat-deg-path", "/position/latitude-deg");
			setprop(view_cfg ~ "/eye-lon-deg-path", "/position/longitude-deg");
			setprop(view_cfg ~ "/eye-alt-ft-path", "/position/altitude-ft");
		}

    		if (views[vn] > 1) {
			setprop(view_cfg ~ "/eye-heading-deg-path", "/orientation/heading-deg");
		}

    		if (views[vn] > 2) {
			setprop(view_cfg ~ "/eye-pitch-deg-path", "/orientation/pitch-deg");
			setprop(view_cfg ~ "/eye-roll-deg-path", "/orientation/roll-deg");
		}

		setprop(view_cfg ~ "/target-lat-deg-path", "/sim/viewer/target/latitude-deg");
		setprop(view_cfg ~ "/target-lon-deg-path", "/sim/viewer/target/longitude-deg");
		setprop(view_cfg ~ "/target-alt-ft-path", "/sim/viewer/target/altitude-ft");
		setprop(view_cfg ~ "/target-heading-deg-path", "/sim/viewer/target/heading-deg");
		setprop(view_cfg ~ "/target-pitch-deg-path", "/sim/viewer/target/pitch-deg");
		setprop(view_cfg ~ "/target-roll-deg-path", "/sim/viewer/target/roll-deg");
	}
}
...
var main = {
	...
	init : func {
		...
		print("Engineer dual control ... initialized");
	...
	update : func {
		...
		foreach (var pilot; mpplayers) {
			if ((pilot.getChild("valid").getValue()) and (pilot.getChild("callsign") != nil) and (pilot.getChild("callsign").getValue() == r_callsign)) {
				if (me.active == 0) {
					# Note: sim/model/path contains the model XML file. And crew members connect all to the pilot not under each other - for now
					if ((pilot.getNode("sim/model/path") != nil) and (pilot.getNode("sim/model/path").getValue() == ADC.pilot_type)) {
						me.active = 1;
						connect(pilot);
					} else {
